...
for num in range(TOTAL_LENGTH//LOAD_LENGTH):
    ...
    for di, dq in zip(i, q):
        carrier_i = np.cos(doppler_nco/DP_NCO_FULL*2*np.pi)#cos(doppler_nco >> 30)
        carrier_q = np.sin(doppler_nco/DP_NCO_FULL*2*np.pi)#sin(doppler_nco >> 30)
        ...
        code_nco_pn = (code_nco_pn + code_nco_omega) % CODE_NCO_FULL_N
...
with open("IQ.txt", "w") as f:
    for n in zip(track_punctual_i, track_punctual_q):
        f.write("{}, {}\r\n".format(n[0],n[1]))
